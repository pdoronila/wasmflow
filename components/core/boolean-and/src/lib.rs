...
impl MetadataGuest for Component {
    fn get_info() -> ComponentInfo {
        ComponentInfo {
            ...
            description: "Returns true only if all inputs are true (logical AND operation)".to_string(),
    ...
    fn get_outputs() -> Vec<PortSpec> {
        vec![PortSpec {
            ...
            data_type: DataType::BoolType,
            optional: false,
            description: "True if all inputs are true, false otherwise".to_string(),
        }]
    }
...
impl ExecutionGuest for Component {
    fn execute(inputs: Vec<(String, Value)>) -> Result<Vec<(String, Value)>, ExecutionError> {
        host::log("debug", "Boolean AND component executing");
...
mod tests {
    ...
    #[test]
    fn test_multiple_inputs_all_true() {
        let inputs = vec![
            ("input1".to_string(), Value::BoolVal(true)),
            ("input2".to_string(), Value::BoolVal(true)),
