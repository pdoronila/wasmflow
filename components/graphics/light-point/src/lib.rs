...
/// Point light data structure (for serialization)
...
impl MetadataGuest for Component {
    fn get_info() -> ComponentInfo {
        ComponentInfo {
            ...
            description: "Point light source that emits light in all directions".to_string(),
...
mod tests {
    ...
    fn test_point_light_basic() {
        ...
        if let Value::StringVal(json) = &result[0].1 {
            let light: PointLightData = serde_json::from_str(json).unwrap();
    ...
    #[test]
    fn test_point_light_clamp_color() {
        let inputs = vec![
            (
                "position".to_string(),
                Value::Vec3Val(Vec3 {
    ...
    #[test]
    fn test_point_light_negative_intensity() {
        let inputs = vec![
            (
                "position".to_string(),
                Value::Vec3Val(Vec3 {
    ...
    #[test]
    fn test_point_light_zero_radius() {
        let inputs = vec![
            (
                "position".to_string(),
                Value::Vec3Val(Vec3 {
    ...
    #[test]
    fn test_point_light_missing_input() {
        let inputs = vec![
            (
                "position".to_string(),
                Value::Vec3Val(Vec3 {
    ...
    #[test]
    fn test_point_light_varying_positions() {
        let inputs = vec![
            (
                "position".to_string(),
                Value::Vec3Val(Vec3 {
            ...
            ("intensity".to_string(), Value::F32Val(2.5)),
            ("radius".to_string(), Value::F32Val(15.7)),
        ];

        let result = Component::execute(inputs).unwrap();
        if let Value::StringVal(json) = &result[0].1 {
            let light: PointLightData = serde_json::from_str(json).unwrap();
            assert_eq!(light.position, [-10.5, 20.3, -5.8]);
            assert_eq!(light.intensity, 2.5);
            assert_eq!(light.radius, 15.7);
        } else {
            panic!("Expected StringVal");
        }
    }
