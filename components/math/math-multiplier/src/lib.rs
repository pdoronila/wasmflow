...
impl ExecutionGuest for Component {
    fn execute(inputs: Vec<(String, Value)>) -> Result<Vec<(String, Value)>, ExecutionError> {
        ...
        // Perform multiplication based on type
        ...
        Ok(vec![("product".to_string(), result)])
    }
}
...
mod tests {
    ...
    fn test_multiply_f32() {
        let inputs = vec![
            ("a".to_string(), Value::F32Val(4.0)),
            ("b".to_string(), Value::F32Val(3.0)),
        ];
        let result = Component::execute(inputs).unwrap();
        assert_eq!(result[0].0, "product");
        match result[0].1 {
            Value::F32Val(v) => assert!((v - 12.0).abs() < 0.001),
    ...
    fn test_multiply_u32() {
        let inputs = vec![
            ("a".to_string(), Value::U32Val(7)),
            ("b".to_string(), Value::U32Val(6)),
        ];
        let result = Component::execute(inputs).unwrap();
        assert_eq!(result[0].0, "product");
        match result[0].1 {
            Value::U32Val(v) => assert_eq!(v, 42),
    ...
    fn test_multiply_by_zero() {
        ...
        assert_eq!(result[0].0, "product");
        match result[0].1 {
            Value::F32Val(v) => assert!((v - 0.0).abs() < 0.001),
            _ => panic!("Expected F32Val"),
        }
    }

    #[test]
    fn test_multiply_type_mismatch() {
        let inputs = vec![
            ("a".to_string(), Value::F32Val(4.0)),
            ("b".to_string(), Value::I32Val(3)),
        ];
    ...
    fn test_multiply_missing_input() {
        let inputs = vec![("a".to_string(), Value::F32Val(4.0))];
        let result = Component::execute(inputs);
        assert!(result.is_err());
    }
}
