...
impl MetadataGuest for Component {
    fn get_info() -> ComponentInfo {
        ComponentInfo {
            ...
            description: "Returns the largest of all input values".to_string(),
    ...
    fn get_outputs() -> Vec<PortSpec> {
        vec![PortSpec {
            ...
            data_type: DataType::F32Type,
            optional: false,
            description: "Maximum of all input values".to_string(),
        }]
    }
...
impl ExecutionGuest for Component {
    fn execute(inputs: Vec<(String, Value)>) -> Result<Vec<(String, Value)>, ExecutionError> {
        host::log("debug", "Maximum component executing");
        ...
        let result = values
            .iter()
            .fold(f32::NEG_INFINITY, |max, &val| if val > max { val } else { max });
