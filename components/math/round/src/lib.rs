...
impl MetadataGuest for Component {
    fn get_info() -> ComponentInfo {
        ComponentInfo {
            ...
            description: "Rounds to the nearest integer (half-way cases round away from zero)".to_string(),
    ...
    fn get_outputs() -> Vec<PortSpec> {
        vec![PortSpec {
            ...
            data_type: DataType::F32Type,
            optional: false,
            description: "Nearest integer to input".to_string(),
        }]
    }
...
impl ExecutionGuest for Component {
    fn execute(inputs: Vec<(String, Value)>) -> Result<Vec<(String, Value)>, ExecutionError> {
        host::log("debug", "Round component executing");
...
mod tests {
    ...
    #[test]
    fn test_round_up() {
        let inputs = vec![("value".to_string(), Value::F32Val(3.5))];
    ...
    #[test]
    fn test_round_down() {
        let inputs = vec![("value".to_string(), Value::F32Val(3.4))];
        let result = Component::execute(inputs).unwrap();
        match &result[0].1 {
    ...
    fn test_round_negative() {
        let inputs = vec![("value".to_string(), Value::F32Val(-2.5))];
        let result = Component::execute(inputs).unwrap();
        match &result[0].1 {
            Value::F32Val(f) => assert!((f - (-3.0)).abs() < 0.001),
            _ => panic!("Expected f32 output"),
