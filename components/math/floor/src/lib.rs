...
impl MetadataGuest for Component {
    fn get_info() -> ComponentInfo {
        ComponentInfo {
            ...
            description: "Rounds down to the nearest integer (towards negative infinity)".to_string(),
    ...
    fn get_inputs() -> Vec<PortSpec> {
        vec![PortSpec {
            ...
            optional: false,
            description: "Number to round down".to_string(),
        }]
    }
    ...
    fn get_outputs() -> Vec<PortSpec> {
        vec![PortSpec {
            ...
            data_type: DataType::F32Type,
            optional: false,
            description: "Largest integer less than or equal to input".to_string(),
        }]
    }
...
impl ExecutionGuest for Component {
    fn execute(inputs: Vec<(String, Value)>) -> Result<Vec<(String, Value)>, ExecutionError> {
        host::log("debug", "Floor component executing");
...
mod tests {
    ...
    fn test_floor_positive() {
        let inputs = vec![("value".to_string(), Value::F32Val(3.7))];
    ...
    fn test_floor_negative() {
        let inputs = vec![("value".to_string(), Value::F32Val(-2.3))];
        let result = Component::execute(inputs).unwrap();
        match &result[0].1 {
            Value::F32Val(f) => assert!((f - (-3.0)).abs() < 0.001),
            _ => panic!("Expected f32 output"),
